    ...
    <div class="row justify-content-center align-items-center g-2">
        ...
        <div class="col-md-8">
            ...
            <div class="table-responsive">
                <table id="detalleingresos" class="table table-striped dt-responsive nowrap" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($detalleingresos as $detalleingreso)
                            <tr class="">
                                <td scope="row">{{ $detalleingreso->id }}</td>
                                <td scope="row">{{ $detalleingreso->proveedor->empresa }}</td>
                                <td scope="row">{{ $detalleingreso->producto->nombre }}</td>
                                <td>{{ $detalleingreso->cantidad }}</td>
                                <td>{{ $detalleingreso->preciocompra }}</td>
                                <td>{{ $detalleingreso->precioventa }}</td>
                                <td>{{ $detalleingreso->fecha }}</td>
                                <td>
                                    <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                        data-bs-target="#edit{{ $detalleingreso->id }}">
                                        Editar
                                    </button>
                                    ...
                                    <form action="{{ route('detalleingresos.destroy', [$detalleingreso->id]) }}" method="POST">
                                        ...
                                        <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                            data-bs-target="#delete{{ $detalleingreso->id }}">
                                            Eliminar
                                        </button>
                                    </form>
                                </td>
                            </tr>
                            @include('detalleingresos.info')
                        @endforeach
                    </tbody>
                </table>
            </div>
            @include('detalleingresos.create')
        </div>
        <div class="col"></div>
    </div>
